...
# Usage: ./train_1000_online.sh [gpu_ids] [mode] [threshold]
...
if [ "$MODE" != "dr" ] && [ "$MODE" != "pseudo" ]; then
    ...
    echo "Usage: ./train_1000_online.sh [gpu_ids] [mode] [threshold]"
    exit 1
fi
...
EXP_NAME="sd15_5k_steps=1000_${MODE}_threshold=${THRESHOLD}"
...
# CUDA_VISIBLE_DEVICES=$GPU_IDS python train_sd15_dpo_online.py "${CMD_ARGS[@]}"
