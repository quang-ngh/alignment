...
def main():
    ...
    unlabeled_train_batch_size = args.train_batch_size * int(len(unlabeled_dataset) / len(labeled_dataset))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (labeled_batch, unlabeled_batch) in enumerate(zip(labeled_dataloader, unlabeled_dataloader)):
            ...
            with accelerator.accumulate(unet):
                ...
                # bsz = min(
                #     len(from_labeled_prompts),
                #     len(from_unlabeled_prompts),
                # )
                ...
                implicit_preds = (logits > 0).sum().float() / logits.size(0)
