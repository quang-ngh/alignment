...
DATASET_NAME_MAPPING = {
    ...
    "drproduck/pickapic-5k-high-margin-sorted": ("jpg_0.jpg", "jpg_1.jpg", "label_0.txt", "original_prompt.txt"),
}
...
def main():
    ...
    # Check if wandb is required but not available
    if args.report_to == "wandb" and not is_wandb_available():
        raise ImportError(
            "wandb is required but not installed. Please install it with: pip install wandb"
        )
    ...
    # dataset = load_dataset("Dragonjinny/FiFA-pickapic-v2", name='FiFA-5k')
    dataset = load_dataset(args.dataset_name)

    # if 'pickapic' in args.dataset_name:
    #     dataset = load_dataset(args.dataset_name, cache_dir=args.cache_dir, data_dir=None)
    #     if args.train_data_path is not None and args.train_data_path != "":
    #         import json
    #         if "caption" in args.train_data_path:
    #             selected_captions = json.load(open(args.train_data_path, "r"))
    #             dataset = dataset.filter(lambda example: example['caption'] in selected_captions)
    #         elif "pair" in args.train_data_path:
    #             selected_captions = json.load(open(args.train_data_path, "r"))
    #             dataset = dataset.filter(lambda example: example['__index_level_0__'] in selected_captions)

    # elif 'HPDv2' in args.dataset_name:
        
    #     dataset = load_dataset(args.dataset_name, cache_dir=args.cache_dir, data_dir=None)

    #     # remove broken samples of HPDv2 dataset
    #     with open('wrong.txt', 'r') as f:
    #         wrong_list = f.readlines()
    #         wrong_list = [int(x.strip()) for x in wrong_list]

    #     #get the length of dataset['train']
    #     length = len(dataset['train'])

    #     # get the correct list -> total indexes - wrong indexes
    #     correct_list = [i for i in range(length) if i not in wrong_list]

    #     dataset['train'] = dataset['train'].select(correct_list)


    #     if args.train_data_path is not None and args.train_data_path != "":
    #         if "caption" in args.train_data_path:
    #             selected_captions = json.load(open(args.train_data_path, "r"))
    #             dataset = dataset.filter(lambda example: example['prompt'] in selected_captions)
    #         elif "pair" in args.train_data_path:
    #             selected_pairs = json.load(open(args.train_data_path, "r"))
    #             dataset['train'] = dataset['train'].select(selected_pairs)
    ...
    image_column_0 = dataset_columns[0]
    image_column_1 = dataset_columns[1]
    label_column_0 = dataset_columns[2]
    caption_column = dataset_columns[3]

    # convert label_column_0 to float
    def convert_labels(examples):
        examples[label_column_0] = [float(x) for x in examples[label_column_0]]
        return examples
    
    dataset[args.split] = dataset[args.split].map(convert_labels, batched=True)
    ...
    #     import json
    #     image_to_reward = json.load(open(args.image_to_reward_path, "r"))
    #     print("Ignoring image_column variable, reading from jpg_0 and jpg_1")
    #     def preprocess_train(examples):
    #         all_pixel_values = []
    #         if 'HPDv2' not in args.dataset_name:
    #             for col_name in ['jpg_0', 'jpg_1']:
    #                 if 'combine_animals' in args.dataset_name:
    #                     images = [Image.open(os.path.join(args.image_path, img_file)).convert("RGB")
    #                             for img_file in examples[col_name]]
    #                 elif 'pickapic' in args.dataset_name:
    ...
    #                 images = [pil_images[image_idx].convert("RGB") for pil_images in examples['image']]
    #                 pixel_values = [train_transforms(image) for image in images]
    #                 all_pixel_values.append(pixel_values)
    ...
    #             reward_values = []
    #         if 'HPD' in args.dataset_name:
    #             if args.use_new_label_0:
    #                 for im_tup, image_path in zip(im_tup_iterator, examples['image_path']):
    #                 # for im_tup, label_0 in zip(im_tup_iterator, examples['label_0']):
    #                     reward_0 = image_to_reward[image_path[0]]
    #                     reward_1 = image_to_reward[image_path[1]]
    #                     label_0 = 1 if reward_0 > reward_1 else 0
    ...
    #                             reward_values.append(torch.Tensor([reward_0, reward_1]))
    #                         else:
    #                             reward_values.append(torch.Tensor([reward_1, reward_0]))
                        
    #             else:
    #                 for im_tup, label, image_path in zip(im_tup_iterator, examples['human_preference'], examples['image_path']):
    #                 # for im_tup, label_0 in zip(im_tup_iterator, examples['label_0']):
    #                     if label[0]==0 and (not args.choice_model): # don't want to flip things if using choice_model for AI feedback
    #                         im_tup = im_tup[::-1]
    #                     combined_im = torch.cat(im_tup, dim=0) # no batch dim
    #                     combined_pixel_values.append(combined_im)
    #                     if args.soft_label_mode:
    ...
    #         else:
    #             if args.use_new_label_0:
    #                 for im_tup, image_0_uid, image_1_uid in zip(im_tup_iterator, examples['image_0_uid'], examples['image_1_uid']):
    #                 # for im_tup, label_0 in zip(im_tup_iterator, examples['label_0']):
    #                     reward_0 = image_to_reward[image_0_uid]
    #                     reward_1 = image_to_reward[image_1_uid]
    #                     label_0 = 1 if reward_0 > reward_1 else 0
    #                     if label_0==0 and (not args.choice_model): # don't want to flip things if using choice_model for AI feedback
    #                         im_tup = im_tup[::-1]
    #                     combined_im = torch.cat(im_tup, dim=0) # no batch dim
    ...
    #             else:
    #                 for im_tup, label_0, image_0_uid, image_1_uid in zip(im_tup_iterator, examples['label_0'], examples['image_0_uid'], examples['image_1_uid']):
    #                 # for im_tup, label_0 in zip(im_tup_iterator, examples['label_0']):
    #                     if label_0==0 and (not args.choice_model): # don't want to flip things if using choice_model for AI feedback
    #                         im_tup = im_tup[::-1]
    #                     combined_im = torch.cat(im_tup, dim=0) # no batch dim
    ...
    #         if args.soft_label_mode:
    #             examples["reward_values"] = reward_values
    ...
    #         if args.soft_label_mode:
    #             return_d["reward_values"] = torch.stack([example["reward_values"] for example in examples]).to(memory_format=torch.contiguous_format).float()
    ...
    def preprocess_train(examples):
        all_pixel_values = []
        for col_name in [image_column_0, image_column_1]:
            images = []
            for im_item in examples[col_name]:
                if isinstance(im_item, bytes):
                    images.append(Image.open(io.BytesIO(im_item)).convert("RGB"))
                elif isinstance(im_item, Image.Image):
                    images.append(im_item.convert("RGB"))
                else:
                    images.append(Image.open(im_item).convert("RGB"))
            pixel_values = [train_transforms(image) for image in images]
            all_pixel_values.append(pixel_values)
        ...
        im_tup_iterator = zip(*all_pixel_values)
        combined_pixel_values = []
        for im_tup, label_0 in zip(im_tup_iterator, examples[label_column_0]):
            if label_0 == 0:
                im_tup = im_tup[::-1]
            combined_im = torch.cat(im_tup, dim=0)  # no batch dim
            combined_pixel_values.append(combined_im)
    ...
    with accelerator.main_process_first():
        ...
        # eliminate no-decisions (0.5-0.5 labels)
        orig_len = dataset[args.split].num_rows
        not_split_idx = [i for i,label_0 in enumerate(dataset[args.split][label_column_0])
                            if label_0 in (0,1) ]
        dataset[args.split] = dataset[args.split].select(not_split_idx)
        new_len = dataset[args.split].num_rows
