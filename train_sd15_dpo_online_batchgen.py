...
def main():
    ...
    def custom_collate_fn(batch):
        """Custom collate function that handles PIL Images by keeping them as lists."""
        from torch.utils.data._utils.collate import default_collate
        
        # Separate PIL Images from other data
        pil_keys = ['image_0_pil', 'image_1_pil']
        pil_data = {key: [item[key] for item in batch] for key in pil_keys if key in batch[0]}
        
        # Remove PIL keys from batch for default collation
        batch_without_pil = [{k: v for k, v in item.items() if k not in pil_keys} for item in batch]
        
        # Use default collate for tensors and other types
        collated = default_collate(batch_without_pil)
        
        # Add PIL Images as lists (not collated)
        collated.update(pil_data)
    ...
    # Initialize CLIP model for density ratio estimation
    clip_model = CLIP(device=accelerator.device)
    
    # Initialize linear classifier for density ratio estimation
    # Input dimension: prompt_embedding_dim + image_embedding_dim
    # CLIP ViT-H-14 has 1024-dim embeddings for both text and image
    clip_embedding_dim = 1024
    classifier_input_dim = clip_embedding_dim + clip_embedding_dim  # prompt + image
    
    density_ratio_classifier = nn.Sequential(
        nn.Linear(classifier_input_dim, 1)
    ).to(accelerator.device)
    density_ratio_classifier.requires_grad_(True)
    
    # Optimizer for classifier (trained stochastically per batch)
    classifier_optimizer = torch.optim.Adam(density_ratio_classifier.parameters(), lr=0.01)
    classifier_criterion = nn.BCEWithLogitsLoss()
    
    # Moving average for weight normalization (handles distributed training and accumulation)
    weight_mean_ema = torch.tensor(1.0, device=accelerator.device)  # Initialize to 1.0
    ema_decay = 0.9  # Exponential moving average decay factor
    
    def create_item_embeddings(prompt_embeddings, image0_embeddings, image1_embeddings):
        """
        Create item embeddings by concatenating prompt and image embeddings.
        For each triplet, creates 2 items: (prompt, image0) and (prompt, image1).
        
        Args:
            prompt_embeddings: (bsz, d_c) tensor
            image0_embeddings: (bsz, d_x) tensor
            image1_embeddings: (bsz, d_x) tensor
            
        Returns:
            item_embeddings: (2*bsz, d_c + d_x) tensor
        """
        # Concatenate prompt with each image
        items_0 = torch.cat([prompt_embeddings, image0_embeddings], dim=1)  # (bsz, d_c + d_x)
        items_1 = torch.cat([prompt_embeddings, image1_embeddings], dim=1)  # (bsz, d_c + d_x)
        
        # Interleave: [item0_0, item0_1, item1_0, item1_1, ...]
        item_embeddings = torch.empty(2 * prompt_embeddings.shape[0], 
                                     prompt_embeddings.shape[1] + image0_embeddings.shape[1],
                                     device=prompt_embeddings.device,
                                     dtype=prompt_embeddings.dtype)
        item_embeddings[0::2] = items_0
        item_embeddings[1::2] = items_1
        
        return item_embeddings
    
    def compute_density_ratio_weights(item_embeddings, weight_mean_ema):
        """
        Compute density ratio weights using the trained classifier.
        
        Args:
            item_embeddings: (2*bsz, d) tensor of item embeddings
            weight_mean_ema: Moving average of weight means (for normalization)
            
        Returns:
            weights: (2*bsz,) tensor of density ratio weights (clipped and normalized)
        """
        density_ratio_classifier.eval()
        with torch.no_grad():
            logits = density_ratio_classifier(item_embeddings).squeeze(-1)
            probs_target = torch.sigmoid(logits)  # P(target | x)
            
            # Compute density ratio: w(x) = P(target|x) / P(source|x) = P(target|x) / (1 - P(target|x))
            weights = probs_target / (1 - probs_target + 1e-10)
            
            # Clip weights to (0.1, 10)
            weights = torch.clamp(weights, min=0.1, max=10.0)
            
            # Normalize using moving average mean (instead of current batch mean)
            weights = weights / (weight_mean_ema + 1e-10)
        
        return weights
    
    #### START ACCELERATOR PREP ####
    unet, optimizer, dataloader, lr_scheduler, density_ratio_classifier, classifier_optimizer = accelerator.prepare(
        unet, optimizer, dataloader, lr_scheduler, density_ratio_classifier, classifier_optimizer
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        metrics_accumulated = {
            ...
            "weight_mean_accumulated": 0.0,
            "weight_std_accumulated": 0.0,
            "weight_mean_ema_accumulated": 0.0,
        }
        for step, batch in enumerate(dataloader):
            # Skip steps until we reach the resumed step
            with accelerator.accumulate(unet, density_ratio_classifier):

                # Get data from good dataset
                prompts = batch["prompt"]
                labeled_images_0 = batch["image_0"]
                labeled_images_1 = batch["image_1"]
                labeled_images_0_pil = batch["image_0_pil"]
                labeled_images_1_pil = batch["image_1_pil"]
                ...
                with torch.no_grad():
                    ...
                    with torch.autocast("cuda", dtype=weight_dtype):
                        ...
                        # Get the images from the pipeline
                        unlabeled_images_pil = pipe(prompt_embeds=encoder_hidden_states.repeat(2, 1, 1), num_inference_steps=20).images
                        # Convert the images to latents
                        # ToTensor() converts PIL to tensor [0, 1], then Normalize to [-1, 1] for VAE
                        image_transform = transforms.Compose([
                            transforms.ToTensor(),
                            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])
                        ])
                        unlabeled_images = torch.stack([image_transform(img).to(accelerator.device, dtype=weight_dtype) for img in unlabeled_images_pil])
                        unlabeled_latents = vae.encode(unlabeled_images).latent_dist.sample() * vae.config.scaling_factor
                        unlabeled_latents_0, unlabeled_latents_1 = unlabeled_latents.chunk(2)
                        unlabeled_images_0_pil, unlabeled_images_1_pil = unlabeled_images_pil[:len(unlabeled_images_pil)//2], unlabeled_images_pil[len(unlabeled_images_pil)//2:]

                    gen_end_time = time.time()
                    gen_time = gen_end_time - gen_start_time
                    
                    # Get CLIP embeddings for density ratio estimation

                    # Get prompt embeddings
                    prompt_embeddings = clip_model.get_text_embeddings(prompts)
                    
                    # Get labeled image embeddings
                    labeled_image0_embeddings = clip_model.get_image_embeddings(labeled_images_0_pil)
                    labeled_image1_embeddings = clip_model.get_image_embeddings(labeled_images_1_pil)
                    
                    # Get unlabeled image embeddings (already PIL images)
                    unlabeled_image0_embeddings = clip_model.get_image_embeddings(unlabeled_images_0_pil)
                    unlabeled_image1_embeddings = clip_model.get_image_embeddings(unlabeled_images_1_pil)
                    
                    # Create item embeddings for density ratio estimation
                    # Source (labeled) item embeddings
                    labeled_item_embeddings = create_item_embeddings(
                        prompt_embeddings, 
                        labeled_image0_embeddings, 
                        labeled_image1_embeddings
                    )
                    
                    # Target (unlabeled) item embeddings
                    unlabeled_item_embeddings = create_item_embeddings(
                        prompt_embeddings,  # Same prompts
                        unlabeled_image0_embeddings,
                        unlabeled_image1_embeddings
                    )
                ...
                # labeled_item_embeddings and unlabeled_item_embeddings each have shape (2*bsz, d)
                # because create_item_embeddings creates 2 items per triplet
                X_clf = torch.cat([labeled_item_embeddings, unlabeled_item_embeddings], dim=0)
                y_clf = torch.cat([
                    torch.zeros(2*bsz, device=accelerator.device, dtype=weight_dtype),  # labeled (source)
                    torch.ones(2*bsz, device=accelerator.device, dtype=weight_dtype)     # unlabeled (target)
                ])
                
                # Training step
                density_ratio_classifier.train()
                logits_clf = density_ratio_classifier(X_clf).squeeze(-1)
                classifier_loss = classifier_criterion(logits_clf, y_clf)
                accelerator.backward(classifier_loss)
                classifier_optimizer.step()
                classifier_optimizer.zero_grad()
                
                # Compute density ratio weights for labeled (source) items
                # First compute raw weights (before normalization) to update moving average
                density_ratio_classifier.eval()
                with torch.no_grad():
                    logits_raw = density_ratio_classifier(labeled_item_embeddings).squeeze(-1)
                    probs_target_raw = torch.sigmoid(logits_raw)
                    weights_raw = probs_target_raw / (1 - probs_target_raw + 1e-10)
                    weights_raw = torch.clamp(weights_raw, min=0.1, max=10.0)
                
                # Update moving average of weight means (only after accumulation, across all processes)
                if accelerator.sync_gradients:
                    # Gather weights from all processes to compute global mean
                    weights_raw_gathered = accelerator.gather(weights_raw)
                    current_mean = weights_raw_gathered.mean()
                    # Update EMA on all processes with the same value (synchronized)
                    weight_mean_ema.data = ema_decay * weight_mean_ema.data + (1 - ema_decay) * current_mean
                    # Ensure EMA is synchronized across all processes (redundant but safe)
                    weight_mean_ema = accelerator.reduce(weight_mean_ema, reduction="mean")
                
                # Compute normalized weights using moving average
                labeled_item_weights = compute_density_ratio_weights(labeled_item_embeddings, weight_mean_ema)
                labeled_item_weights_reshaped = labeled_item_weights.view(bsz, 2)
                labeled_triplet_weights = torch.prod(labeled_item_weights_reshaped, dim=1)  # (bsz,)
                
                # Compute weight statistics for diagnostics
                weight_mean = labeled_triplet_weights.mean()
                weight_std = labeled_triplet_weights.std()
                #########################################################
                ...
                labeled_loss = binary_cross_entropy_with_logits(labeled_logits, labeled_prefs, reduction='none')
                if args.no_dr:
                    loss = (labeled_loss.sum() + pseudo_unlabeled_loss.sum()) / (bsz + bsz)
                else:
                    augmented_loss = (labeled_loss - pseudo_labeled_loss) * labeled_triplet_weights
                    loss = (pseudo_labeled_loss.sum() + pseudo_unlabeled_loss.sum()) / (bsz + bsz) + curriculum_weight * augmented_loss.sum() / bsz
                ...
                avg_weight_mean = accelerator.gather(weight_mean.detach()).mean().item()
                avg_weight_std = accelerator.gather(weight_std.detach()).mean().item()
                avg_weight_mean_ema = accelerator.gather(weight_mean_ema.detach()).mean().item()
                ...
                current_metrics = {
                    ...
                    "weight_mean_accumulated": avg_weight_mean,
                    "weight_std_accumulated": avg_weight_std,
                    "weight_mean_ema_accumulated": avg_weight_mean_ema,
                }
            ...
            if accelerator.sync_gradients:
                ...
                diagnostic_metrics = {
                    "labeled_margin_accumulated", 
                    "unlabeled_margin_accumulated", 
                    "image_generation_time_accumulated",
                    "weight_mean_accumulated",
                    "weight_std_accumulated",
                    "weight_mean_ema_accumulated"
                }
                ...
                if accelerator.is_main_process:
                    ...
                    with torch.no_grad():
                        # Decode latents (ensure dtype matches VAE dtype)
                        decoded_0 = vae.decode((unlabeled_latents_0[0:1] / vae.config.scaling_factor).to(dtype=weight_dtype)).sample
                        decoded_1 = vae.decode((unlabeled_latents_1[0:1] / vae.config.scaling_factor).to(dtype=weight_dtype)).sample
                        # Denormalize from [-1, 1] to [0, 1]
