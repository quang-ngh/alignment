...
def parse_args():
    ...
    parser.add_argument(
        "--external_model", type=str, required=True, help="External model for pseudo-labeling"
    )
...
def main():
    ...
    labeled_dataset = BaseDataset(manifest=args.labeled_manifest, image_dir=args.train_data_dir, resolution=resolution, external_model=args.external_model)
    unlabeled_dataset = BaseDataset(manifest=args.unlabeled_manifest, image_dir=args.train_data_dir, resolution=resolution, external_model=args.external_model)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, (labeled_batch, unlabeled_batch) in enumerate(zip(labeled_dataloader, unlabeled_dataloader)):
            ...
            with accelerator.accumulate(unet):
                ...
                labeled_pseudo_prefs = labeled_batch[args.external_model].to(accelerator.device, dtype=weight_dtype)
                ...
                unlabeled_pseudo_prefs = unlabeled_batch[args.external_model].to(accelerator.device, dtype=weight_dtype)
                ...
                pseudo_labeled_loss = binary_cross_entropy_with_logits(
                        labeled_logits,
                        labeled_pseudo_prefs,
                        reduction='mean',
                )
                pseudo_unlabeled_loss = binary_cross_entropy_with_logits(
                        unlabeled_logits,
                        unlabeled_pseudo_prefs,
                        reduction='mean',
                )
                labeled_loss = binary_cross_entropy_with_logits(labeled_logits, labeled_prefs, reduction='mean')
                if args.no_dr:
            ...
            if accelerator.sync_gradients:
                ...
                diagnostic_metrics = {"pseudo_unlabeled_acc_accumulated"}
